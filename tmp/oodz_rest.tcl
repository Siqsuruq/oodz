nx::Class create apiin -superclass oodz_superclass {
	...
	:public method inf {args} {
		switch [lindex $args 0] {
			...
			# }
			default {
				return [dict create obj_domain [::oodzConf srv get] obj_uuid ${:obj_uuid} obj_ts ${:obj_ts}]
			}
		}
	}
	...
	:public method answer_request {args} {
		if {[[self] cget -ssl] ne "nsssl"} {
			...
			if {[file isdirectory [file join [::oodzConf srvpath get] [set [::oodzConf srv get]::mod_dir] $resource]] == 1 && [info proc ::${resource}::api] ne ""} {

nx::Class create oodz_datetimeclass -superclass oodz_baseclass {
	...
	:public method tomorrow {} {
		return [clock format [clock scan "tomorrow"] -format ${:date_format} -timezone "${:timezone}"]
	}

	:public method yesterday {} {
		return [clock format [clock scan "yesterday"] -format ${:date_format} -timezone "${:timezone}"]
	}
	...
	:public method age {date1} {
		set now [clock format [clock scan [: date] -format ${:date_format} -timezone "${:timezone}"] -format %Y]
		return [expr $now - [clock format [clock scan $date1] -format %Y]]
	}
...
oodz_datetimeclass create oodzTime -oodzConf oodzConf